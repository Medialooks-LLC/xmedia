...
/**
 * @brief Interface for media stat & props (TODO)
 */
class IMediaStatAndProps {
public:
    USING_PTRS(IMediaStatAndProps)

    virtual ~IMediaStatAndProps() = default;
...
public:
    ...
    /**
     * @brief Change the property of the media unit using a XPath expression and a new value.
    ...
    /**
     * @brief Change the properties of the media unit using a vector of key-value pairs and a base XPath
     * expression.
...
/**
 * @brief Interface for media handlers input.
 */
class IMediaInput {
public:
    USING_PTRS(IMediaInput)

    virtual ~IMediaInput() = default;

public:
    /**
     * @brief Get a list of input streams media props (or recent input media objects)
     * @param _add_to An optional vector to which the input streams will be added.
     * @return A vector of input stream properties.
     */
    virtual MediaUnitsVec Inputs(MediaUnitsVec&& _add_to = {}) const = 0;

    /**
     * @brief Get to input stream media props/media obejct (recent output media objects)
     * @param _stream_uid An output streams uid.
     * @return Output stream media properties/recent object.
     */
    virtual IMediaUnit::SPtrC Input(const uint64_t _stream_uid) const = 0;
     ...
     */
    virtual std::error_code MediaPut(const IMediaUnit::SPtrC& _media, const INode::SPtrC& _hints = {}) = 0;
};

/**
 * @brief Interface for media handlers output.
 */
class IMediaOutput {
public:
    USING_PTRS(IMediaOutput)

    virtual ~IMediaOutput() = default;

public:
    /**
     * @brief Get a list of output streams media props (or recent output media objects)
     * @param _add_to An optional vector to which the output streams will be added.
     * @return A vector of output stream properties.
     */
    virtual MediaUnitsVec Outputs(MediaUnitsVec&& _add_to = {}) const = 0;

    /**
     * @brief Get to output stream media props/media obejct (recent output media objects)
     * @param _stream_uid An output streams uid.
     * @return Output stream media properties/recent object.
     */
    virtual IMediaUnit::SPtrC Output(const uint64_t _stream_uid) const = 0;
 ...
 */
class IMediaHandler: public IObject, public IMediaInput, public IMediaOutput {
public:
...
public:
     ...
     */
    virtual xbase::XResult<MediaUnitsVec> Init(InitParamsVariant&& _init_url_or_func,
                                               const INode::SPtrC& _init_props          = {},
                                               MediaUnitsVec&&     _input_streams_props = {}) = 0;
